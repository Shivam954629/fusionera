...
export default function Section1() {
  ...
  return (
    <section id="featured" className="relative isolate w-full overflow-hidden py-16 md:py-20">
      ...
      <div className="mx-auto grid w-[92%] max-w-7xl items-center gap-12 md:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="px-4 md:px-8">
          ...
          <div className="mt-5">
            <h3 className="text-2xl font-semibold text-[#ffe3a6]">04–07 July 2026</h3>
            <h4 className="mt-2 text-xl leading-relaxed text-[#d9e7ff]">
              Bharat Mandapam, Pragati Maidan,<br />New Delhi, India
            </h4>
          </div>
            ...
            <div className="mt-4 grid grid-cols-4 gap-3 text-center">
              {[
                ...
                <div key={item.label} className="rounded-2xl border border-white/10 bg-black/20 p-3">
                  ...
                  <p className="mt-1 text-[10px] uppercase tracking-[0.15em] text-[#d9e7ff]">{item.label}</p>
                </div>
              ))}
            </div>
            <a
                ...
                {textileSlides.map((slide, index) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      activeTextileSlide === index ? "w-8 bg-white" : "w-2 bg-white/40"
                    }`}
                  />
                ))}
              </div>
