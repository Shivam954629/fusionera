  ...
  if (error || !visitor)
    return (
      <section className="min-h-screen flex items-center justify-center px-4 bg-[#5B9BD5]">
        <div className="w-full max-w-md rounded-2xl bg-white p-6 text-center shadow-sm" style={{ border: "1px solid rgba(0,80,157,0.15)" }}>
          <h1 className="text-xl font-bold text-[#00509d]">Unable to load your pass</h1>
          <p className="mt-2 text-sm text-gray-500">{error || "Please sign in again to continue."}</p>
          <button
      ...
      <div className="mx-auto w-full max-w-lg px-4 py-10 sm:px-6">
        <div className="rounded-2xl overflow-hidden shadow-xl" style={{ border: "1px solid rgba(0,80,157,0.2)" }}>
          ...
          <div className="bg-white px-6 pt-6 pb-8 space-y-4">
            ...
            {visitor.qrCode ? (
              <div className="rounded-xl p-5 text-center" style={{ background: "#f0f6ff", border: "1px solid rgba(0,80,157,0.12)" }}>
                <p className="text-xs font-bold uppercase tracking-widest text-[#00509d] mb-3">
                  Scan at Entry Gate
                </p>
                <div className="inline-block p-3 bg-white rounded-xl shadow-sm">
                  <img src={visitor.qrCode} alt="Entry QR Code" className="w-48 h-48 md:w-56 md:h-56" />
                </div>
                <p className="text-xs text-gray-400 mt-2">Show this QR to the gate attendant</p>
                <a
                  href={visitor.qrCode}
                  download={`fusionera-pass-${visitor.regNo}.png`}
                  className="inline-block mt-3 px-5 py-2 rounded-xl text-white text-sm font-semibold transition hover:opacity-90"
                  style={{ background: "#00509d" }}
                >
                  📥 Download Pass
                </a>
              ...
              <div className="rounded-xl p-4 text-center" style={{ background: "#f0f6ff", border: "1px solid rgba(0,80,157,0.12)" }}>
                <p className="text-sm text-gray-500">QR code will appear after full registration is complete.</p>
              </div>
            )}

            {/* Reg No */}
            <div className="rounded-xl p-4 text-center" style={{ background: "linear-gradient(135deg,#002f6c,#00509d)" }}>
              <p className="text-xs font-extrabold uppercase tracking-widest mb-1 text-white/70">Registration Number</p>
              <p className="text-3xl font-black text-white tracking-widest">{visitor.regNo}</p>
            </div>
