...
export default function NewsletterPage() {
  ...
  if (loading) {
    return (
      <section
        className="w-full py-8 md:py-12 bg-[#cae9ff] flex items-center justify-center"
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-10">
        <div className="relative grid gap-6 p-4 text-[#00509d] sm:p-6 md:p-8">
          <div>
            ...
            {data.title && (
              <p className="mt-4 text-lg font-semibold text-[#00509d]">
                {data.title}
              </p>
            )}
            {data.subtitle && (
            ...
            {data.content && (
              <p className="mt-4 text-md leading-7 whitespace-pre-wrap  text-black">
                {data.content}
              </p>
            )}
          </div>
