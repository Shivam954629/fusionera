...
export default function ExhibitorManualPage() {
  return (
    ...
    >
      <h2 className="mt-4 text-2xl font-bold md:text-3xl" style={{ color: "#0c1148" }}>Exhibitor Manual</h2>
      ...
      <h3 className="mt-8 text-xl font-bold" style={{ color: "#0c1148" }}>Step-by-Step Guide</h3>
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {steps.map((s) => (
          <div key={s.no} className="rounded-xl p-5 bg-white" style={{ border: "1px solid rgba(12,17,72,0.1)" }}>
            <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white text-sm font-black mb-3" style={{ background: "#0c1148" }}>{s.no}</div>
            <h4 className="font-bold text-sm mb-1" style={{ color: "#0c1148" }}>{s.title}</h4>
            <p className="text-xs leading-6 text-gray-500">{s.desc}</p>
          </div>
        ))}
      </div>

      <h3 className="mt-10 text-xl font-bold" style={{ color: "#0c1148" }}>Key Show Information</h3>
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={item.label} className="rounded-xl p-4 text-center bg-white" style={{ border: "1px solid rgba(12,17,72,0.1)" }}>
            <p className="text-xs text-gray-500 mb-1">{item.label}</p>
            <p className="font-bold text-sm" style={{ color: "#0c1148" }}>{item.value}</p>
          </div>
        ))}
      </div>

      <h3 className="mt-10 text-xl font-bold" style={{ color: "#0c1148" }}>Frequently Asked Questions</h3>
      <div className="mt-4 space-y-3">
        {faqs.map((f) => (
          <div key={f.q} className="rounded-xl p-5 bg-white" style={{ border: "1px solid rgba(12,17,72,0.08)" }}>
            <p className="font-semibold text-sm" style={{ color: "#0c1148" }}>{f.q}</p>
