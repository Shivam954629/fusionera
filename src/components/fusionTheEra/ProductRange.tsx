...
export default function ProductRange() {
  return (
    <section id="range" className="w-full py-8 md:py-12 reveal-on-scroll reveal-zoom bg-[#cae9ff]" data-reveal-delay="50">
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {CATEGORIES.map((c) => (
                <div
                  ...
                  style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(0,80,157,0.15)" }}
                >
                  <span className="text-2xl flex-shrink-0">{c.icon}</span>
                  <span className="font-semibold text-sm text-[#00509d]">{c.label}</span>
                </div>
              ))}
            </div>
          </div>
